...
5. **只用已收盘的 5 分钟 K 线计算 RSI。** 当前 5 分钟周期的半根 K 线会持续变化，不参与信号判断；OHLCV 使用已验证可返回 Pump 代币数据的 Birdeye v2 `/defi/ohlcv`，并用至少 100 根 K 线暖机 Wilder RSI。不会按“波动小于 0.5%”删除 K 线，因为波幅无法证明一根 K 线是伪造数据。
...
- OHLCV 使用 v2 的 `time_from/time_to` 参数；不发送会令该端点返回空数据的 v3 专属参数。空响应会记录 `birdeye_ohlcv_empty` 错误，且不会把旧 K 线伪装成一次成功刷新。
- v2 K 线使用独立数据版本；首次部署会为每个代币重新暖机约 105 根 5 分钟 K 线并覆盖同时间戳旧值，避免历史坏数据继续影响 RSI。
