...
const schema = z.object({
  ...
  RSI_BUY_BELOW: numberFromEnv(30).pipe(z.number().min(0).max(100)),
  RSI_SELL_CROSS_DOWN: numberFromEnv(70).pipe(z.number().min(0).max(100)),
  ...
  TRAILING_ACTIVATE_PROFIT_PERCENT: numberFromEnv(20).pipe(z.number().min(0).max(1000)),
